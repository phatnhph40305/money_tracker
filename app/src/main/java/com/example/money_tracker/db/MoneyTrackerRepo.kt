...
class MoneyTrackerRepo(private val moneyTrackerDb: MoneyTrackerDb) {
    ...
    suspend fun insert(taskLog: TaskLog) = withContext(Dispatchers.IO) {
        ...
        ContentValues().apply {
            put(DbConfig.TaskLog.COL_NAME, taskLog.name)
        ...
        cursor?.let {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    val idTaskName = cursor.getColumnIndex(DbConfig.TaskLog.COL_NAME)
    ...
    suspend fun update(taskLog: TaskLog, taskId: Int) = withContext(Dispatchers.IO) {
